# Multivariate Generalized Estimating Equations (GEE) for Mixed Phenotypes
...
# Function to simulate mixed phenotype data
# n: sample size
# p: probability for the genotype variable (G)
...
sim.dataset.full <- function(n, p, f, B, mu, S) {
  ...
  simulated_data2 <- data.frame(
    ...
    mutate(GE = G * E)  # Add interaction term (GE)
  
  # Generate phenotype data (Y1, Y2)
  X <- as.matrix(simulated_data2)
  YY <- X %*% B + mvrnorm(n, mu = mu, Sigma = S)
  colnames(YY) <- c("Y1", "Y2")
  
  # Convert Y2 into a binary variable based on a cutoff (top 20%)
...
# Generalized Estimating Equations (GEE) function for the full model
# n: sample size
# response_mat: matrix of response variables (Y1, Y2)
# design_mat: design matrix for predictors (G, E, GE)
# beta: initial parameter estimates
# r: correlation coefficient between residuals
# phi: scale parameter (variance of residuals)
...
GEE <- function(n, response_mat, design_mat, beta, r, phi) {
  ...
  for (i in 1:n) {
    ...
    mu1 <- as.numeric(X[i, ] %*% beta[1:4]) # Added as.numeric() to ensure scalar
    ...
    Vi <- matrix(c(phi^2, r * phi * sqrt(var2), r * phi * sqrt(var2), var2), 2, 2)
...
# GEE function for marginal tests
GEE.G <- function(n, response_mat, design_mat, beta.G, r, phi) {
  Y1 <- response_mat[, 1]
  Y2 <- response_mat[, 2]
  X <- as.matrix(design_mat)
  
  # Regularization by adding small value at the diagonal
  epsi <- 1e-5
  ...
  for (i in 1:n) {
    mu1 <- as.numeric(X[i, ] %*% beta.G[1:2])  # Added as.numeric() to ensure scalar
...
# Power calculation function for multivariate GEE tests
...
power.fn <- function(n.sims, n, p, f, B, mu, S, alpha) {
  ...
  for (dataset.i in 1:n.sims) {
    ...
    model1 <- glm(Y1 ~ G, data = Data, family = "gaussian")
    power$sig.Y1.G[dataset.i] <- tidy(model1)$p.value[2] < alpha
    model2 <- glm(Y2 ~ G, data = Data, family = "binomial") 
    ...
    repeat {
      ...
      pearson_resid1<-pheno[1]-X%*%beta.newGEE.G[1:2]
      # calculate Pearson residuals for Binary Phenotype
    ...
    repeat {
      ...
      # calculate Pearson residuals for Binary Phenotype
      pearson_resid1<-pheno[1]-X%*%beta.newGEE[1:4]
      # calculate Pearson residuals for Binary Phenotype
